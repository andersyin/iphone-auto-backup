...
#
# v3.6 修复（2026-07-20 实跑教训）：
#   - exiftool -s 输出带标签名前缀，grep '^[0-9]{4}' 恒不匹配 → 改 -s3 只输出值
#     （旧版所有文件都落到同步当日文件夹的根因）
#   - AFC 下载可能在整 MiB 处静默截断 → 下载后必须比对设备侧 st_size，失败重试 1 次
#   - 无 EXIF 文件（截图/App 保存）回退链新增设备侧 st_mtime；最终兜底改 _unknown_date，
#     不再用本地 mtime（=下载时间）冒充拍摄日期
#   - 纯空白文件名（如 " .mov"）拒收
#   - 新增 device_seen 增量索引：设备路径+大小未变 → 不下载直接跳过
#     （旧版每次同步都要全量重新下载所有文件才能算哈希去重）
#
# v3.3 修复：
...
HASH_INDEX="$HOME/.iphone_video_backup_v3.hashes"
DEVICE_SEEN="$HOME/.iphone_video_backup_v3.device_seen"
TMP_DIR="/tmp/iphone_video_backup_v3_tmp"

# ---------- 锁（mkdir 原子锁，macOS/Linux 通用，消除 TOCTOU 竞态）----------
LOCKDIR="/tmp/iphone_video_backup_v3.lockdir"
if ! mkdir "$LOCKDIR" 2>/dev/null; then
    _lock_pid=$(cat "$LOCKDIR/pid" 2>/dev/null || echo "")
    if [[ -n "$_lock_pid" ]] && kill -0 "$_lock_pid" 2>/dev/null; then
        echo "[$(date '+%H:%M:%S')] [INFO] 视频备份已在运行 (PID $_lock_pid)，退出"
        exit 0
    fi
    # 进程已死，清理过期锁目录
    rm -rf "$LOCKDIR"
    if ! mkdir "$LOCKDIR" 2>/dev/null; then
        echo "[$(date '+%H:%M:%S')] [INFO] 视频备份锁已被其他进程接管，退出"
        exit 0
    fi
fi
echo $$ > "$LOCKDIR/pid"
...
other_iphone_backup_running() {
    pgrep -f "backup_photos_v3.sh" >/dev/null 2>&1
}
...
run_media_index_if_ready() {
    ...
    if [[ "$failed" -gt 0 ]]; then
        ...
        notify "iPhone 媒体索引未启动" "视频备份有 $failed 个失败,先处理失败文件。"
        return 0
    fi
    if [[ "$copied" -gt 0 ]]; then
        echo "video $(date '+%Y-%m-%dT%H:%M:%S') copied=$copied" >> "$pending_flag"
    fi
    ...
    if other_iphone_backup_running; then
        ...
        notify "iPhone 生产线等待" "视频已备份,等待照片备份完成后统一刷新索引。"
        return 0
    fi
    ...
    notify "iPhone 媒体索引失败" "视频备份已完成,但索引刷新失败;请查看日志后手动重跑媒体索引。"
    rm -f "$pending_flag"
    rm -rf "$index_lock"
    return 1
}
...
# ---------- 拍摄日期读取（EXIF → 文件名 → 设备侧mtime → _unknown_date）----------
# $2 = 设备侧 st_mtime（秒，可空）。
...
get_video_date() {
    ...
    local ds
    ds=$(expr "$bn" : 'IMG_\([0-9]\{8\}\)_' 2>/dev/null || true)
    if [[ -n "$ds" && ${#ds} -eq 8 ]]; then
        echo "${ds:0:4}-${ds:4:2}-${ds:6:2}"; return
    fi

    # 设备侧修改时间：截图/App 保存的无 EXIF 文件，这是最接近真实日期的来源
...
# $2 = 设备侧 st_mtime（秒，可空），用于无 EXIF 的视频回退。
...
get_video_timestamp() {
    ...
    ts=$("$EXIFTOOL" -MediaCreateDate -s3 -d "%Y%m%d%H%M.%S" "$fp" 2>/dev/null | head -1)
    if [[ -n "$ts" && "$ts" =~ ^[0-9]{12}\.[0-9]{2}$ ]]; then
        echo "$ts"; return
    fi
    # 无 EXIF → 设备侧 st_mtime（真实保存日）
    if [[ -n "$dev_mtime" && "$dev_mtime" =~ ^[0-9]{9,10}$ ]]; then
...
tmp_path_for() {
    ...
    # 拒绝纯空白主名（如 " .mov"）：此类名多为传输残片，且肉眼不可见（2026-07-20 实跑教训）
    local stem="${base%.*}"
...
cleanup_target_dirs() {
    ...
    done < <(find "$BACKUP_ROOT" -mindepth 1 -type d -empty 2>/dev/null || true)
...
# ---------- 统计备份目录 ----------
count_backup_files() {
    find "$BACKUP_ROOT" -type f 2>/dev/null | wc -l | tr -d ' '
}
...
main() {
    ...
    for entry in "${iphone_videos[@]}"; do
        ...
        # 读取视频日期
        local video_date; video_date=$(get_video_date "$tmp_path" "$dev_mtime")

        # 归档
        local target_dir="$BACKUP_ROOT/$video_date"
        mkdir -p "$target_dir"
        ...
        if mv "$tmp_path" "$target_path" 2>/dev/null; then
            new_files+=("$video_date/$new_name")
            hash_record "$h"
            [[ -n "$dev_size" ]] && device_record "${src_path}|${dev_size}"
            ((copied++)) || true
            # 2026-07-21: 设文件 mtime = EXIF 拍摄时间，使下游索引读到拍摄日期而非下载日期
            local ts; ts=$(get_video_timestamp "$target_path" "$dev_mtime")
            if [[ -n "$ts" ]]; then
        ...
        clear_bar
        prok "$fname → $video_date/$new_name"
    done
    ...
    echo "======================================================================"
    printf "%-12s %s\n" "同步文件夹:" "$BACKUP_ROOT"
    printf "%-12s %s\n" "  共计:" "$total_backup 个视频文件"
    printf "%-12s ${GREEN}%s${NC}\n" "  新增:" "$copied 个视频"
    printf "%-12s ${YELLOW}%s${NC}\n" "  跳过:" "$skipped 个视频（已存在/内容重复，其中 $prefilter_skipped 个设备侧未变、免下载秒过）"
    printf "%-12s ${RED}%s${NC}\n" "  失败:" "$failed 个视频"
    echo ""
    ...
    if (( skipped == 0 && copied == 0 && total > 0 )); then
        echo "  1. 所有文件均已备份，无需重复操作"
    fi
    if (( total == 0 )); then
        echo "  1. iPhone DCIM 中未发现视频，请确认视频存储在本地而非 iCloud"
    fi
    if (( copied > 0 )); then
        echo "  1. 定期检查 $BACKUP_ROOT 目录占用空间"
    fi
    echo "======================================================================"
    echo ""
    ...
    notify "iPhone 视频备份完成" "新增: ${copied} | 跳过: ${skipped} | 失败: ${failed}"
